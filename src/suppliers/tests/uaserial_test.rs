...
async fn should_load_content_details_for_movie() {
    let sup = get_supplier(NAME).unwrap();
    let res = AllContentSuppliers::get_content_details(&sup, "movie-the-terminator".into())
        .await
        .unwrap();
    println!("{res:#?}");
}
...
async fn should_load_content_details_for_tv_show() {
    let sup = get_supplier(NAME).unwrap();
    let res =
        AllContentSuppliers::get_content_details(&sup, "universal-basic-guys/season-1".into())
            .await
            .unwrap();
    println!("{res:#?}");
}
