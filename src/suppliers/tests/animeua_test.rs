...
async fn should_load_channel() {
    ...
    let res = AllContentSuppliers::load_channel(&sup, "ТОП 100".into(), 2)
        .await
        .unwrap();
    println!("{res:#?}");
}
...
async fn should_search() {
    let sup = get_supplier(NAME).unwrap();
    let res = AllContentSuppliers::search(&sup, "Доктор Стоун".into(), vec![])
        .await
        .unwrap();
    println!("{res:#?}");
}
...
async fn should_load_content_details() {
    let sup = get_supplier(NAME).unwrap();
    let res = AllContentSuppliers::get_content_details(&sup, "7633-dr-stone-4".into())
        .await
        .unwrap();
    println!("{res:#?}");
}
