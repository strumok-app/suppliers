...
const VIDUP_URL: &str = "https://vidup.to";

// enc-dec.app request/response types for vidup
...
async fn vidup_dec_servers(text: &str) -> anyhow::Result<Vec<Server>> {
    let url = format!("{ENC_DEC_APP_URL}/api/dec-vidup");
...
async fn vidup_dec_stream(text: &str) -> anyhow::Result<StreamResult> {
    let url = format!("{ENC_DEC_APP_URL}/api/dec-vidup");
...
pub async fn extract(params: &SourceParams) -> anyhow::Result<Vec<ContentMediaItemSource>> {
    ...
    let page_url = match &params.ep {
        Some(ep) => format!("{VIDUP_URL}/tv/{tmdb_id}/{}/{}/", ep.s, ep.e),
        None => format!("{VIDUP_URL}/movie/{tmdb_id}/"),
    };
    ...
    let servers = vidup_dec_servers(&servers_encrypted).await?;
    ...
    let server_futures = servers.into_iter().enumerate().map(|(idx, server)| {
        ...
        async move {
            match load_server_stream(client, stream_url, &server.data, token, idx).await {
                ...
                Err(err) => {
                    warn!("[vidup] server {} failed: {err}", idx + 1);
                    vec![]
                }
            }
        }
...
fn extract_text(html: &str) -> anyhow::Result<String> {
    ...
    let caps = re
        .captures(html)
        .ok_or_else(|| anyhow!("[vidup] failed to extract text from page"))?;
...
async fn load_server_stream(
    ...
    // Decrypt stream data
    let stream_data = vidup_dec_stream(&stream_encrypted).await?;
    ...
    let headers = if stream_data.no_referrer {
        ...
        Some(HashMap::from([
            ("Origin".to_string(), VIDUP_URL.to_string()),
            ("Referer".to_string(), format!("{VIDUP_URL}/")),
        ]))
    };

    sources.push(ContentMediaItemSource::Video {
        link: stream_data.url,
        description: format!("[VidUp] Server {}", idx + 1),
        headers,
        hls_proxy: false,
    });
...
mod tests {
    ...
    async fn vidup_should_extract_tv() {
    ...
    #[test_log::test(tokio::test)]
    async fn vidup_should_extract_movie() {
        let res = extract(&SourceParams {
            id: 533535,
            imdb_id: None,
            ep: None,
